...
// Modified by EolaFam1828 (2026) — Updated Wire dependency injection for MCP controllers.
...
func initSystem(ctx context.Context, config *types.Config) (*server.System, error) {
	...
	// Error Bridge (connects errors → auto-remediation)
	errorBridgeInstance := errorbridge.NewBridge(remediationStore, true)
	errorTrackerCtrl.SetErrorBridge(errorBridgeInstance)

	soloDevModules := &router2.SoloDevModules{
