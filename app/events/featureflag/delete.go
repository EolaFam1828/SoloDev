...
func (r *Reporter) Deleted(ctx context.Context, payload *DeletedPayload) {
	...
	if err != nil {
		log.Ctx(ctx).Err(err).Msgf("failed to send feature flag deleted event")
		return
	}

	log.Ctx(ctx).Debug().Msgf("reported feature flag deleted event with id '%s'", eventID)
}

func (r *Reader) RegisterDeleted(fn events.HandlerFunc[*DeletedPayload],
