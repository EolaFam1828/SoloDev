...
// Modified by EolaFam1828 (2026) — Added RemediationStore interface for SoloDev AI remediation module.
//
...
type (
	...
	FeatureFlagStore interface {
		...
		Create(ctx context.Context, featureFlag *types.FeatureFlag) error

		// Update updates an existing feature flag.
		Update(ctx context.Context, featureFlag *types.FeatureFlag) error

		// Find finds a feature flag by id.
		Find(ctx context.Context, id int64) (*types.FeatureFlag, error)

		// FindByIdentifier finds a feature flag by space id and identifier.
		FindByIdentifier(ctx context.Context, spaceID int64, identifier string) (*types.FeatureFlag, error)

		// List lists feature flags in a space.
		List(ctx context.Context, spaceID int64, filter *types.FeatureFlagFilter) ([]*types.FeatureFlag, error)

		// Count returns the count of feature flags in a space.
		Count(ctx context.Context, spaceID int64, filter *types.FeatureFlagFilter) (int64, error)

		// Delete deletes a feature flag.
		Delete(ctx context.Context, id int64) error
	}

	TechDebtStore interface {
		// Create creates a new technical debt item.
		Create(ctx context.Context, techDebt *types.TechDebt) error

		// Update updates an existing technical debt item.
		Update(ctx context.Context, techDebt *types.TechDebt) error

		// Find finds a technical debt item by id.
		Find(ctx context.Context, id int64) (*types.TechDebt, error)

		// FindByIdentifier finds a technical debt item by space ID and identifier.
		FindByIdentifier(ctx context.Context, spaceID int64, identifier string) (*types.TechDebt, error)

		// List lists technical debt items based on the provided filter.
		List(ctx context.Context, spaceID int64, filter *types.TechDebtFilter) ([]*types.TechDebt, error)

		// Count returns the count of technical debt items matching the filter.
		Count(ctx context.Context, spaceID int64, filter *types.TechDebtFilter) (int64, error)

		// Delete deletes a technical debt item.
		Delete(ctx context.Context, id int64) error

		// Summary returns aggregated statistics for technical debt items.
		Summary(ctx context.Context, spaceID int64, filter *types.TechDebtFilter) (*types.TechDebtSummary, error)
	}

	// SecurityScanStore defines the security scan data storage.
	SecurityScanStore interface {
		// Create creates a new security scan.
		Create(ctx context.Context, scan *types.ScanResult) error

		// Find finds a security scan by id.
		Find(ctx context.Context, id int64) (*types.ScanResult, error)

		// FindByIdentifier finds a security scan by identifier for a repo.
		FindByIdentifier(ctx context.Context, repoID int64, identifier string) (*types.ScanResult, error)
	...
	// ScanFindingStore defines the scan finding data storage.
	ScanFindingStore interface {
		// Create creates a new scan finding.
		Create(ctx context.Context, finding *types.ScanFinding) error

		// Find finds a scan finding by id.
		Find(ctx context.Context, id int64) (*types.ScanFinding, error)

		// ListByScan lists all findings for a specific scan.
		ListByScan(ctx context.Context, scanID int64, filter *types.ScanFindingFilter) ([]*types.ScanFinding, int64, error)

		// Update updates a scan finding.
		Update(ctx context.Context, finding *types.ScanFinding) error
	...
	// QualityRuleStore defines the quality rule data storage.
	QualityRuleStore interface {
		// Create creates a new quality rule.
		Create(ctx context.Context, rule *types.QualityRule) error

		// Update updates an existing quality rule.
		Update(ctx context.Context, rule *types.QualityRule) error

		// Find finds a quality rule by id.
		Find(ctx context.Context, id int64) (*types.QualityRule, error)

		// FindByIdentifier finds a quality rule by space id and identifier.
		FindByIdentifier(ctx context.Context, spaceID int64, identifier string) (*types.QualityRule, error)

		// List lists quality rules in a space.
		List(ctx context.Context, spaceID int64, filter *types.QualityRuleFilter) ([]*types.QualityRule, error)

		// Count returns the count of quality rules in a space.
		Count(ctx context.Context, spaceID int64, filter *types.QualityRuleFilter) (int64, error)

		// Delete deletes a quality rule.
		Delete(ctx context.Context, id int64) error
	}

	// QualityEvaluationStore defines the quality evaluation data storage.
	QualityEvaluationStore interface {
		// Create creates a new quality evaluation.
		Create(ctx context.Context, eval *types.QualityEvaluation) error

		// Find finds a quality evaluation by id.
		Find(ctx context.Context, id int64) (*types.QualityEvaluation, error)

		// FindByIdentifier finds a quality evaluation by identifier.
		FindByIdentifier(ctx context.Context, identifier string) (*types.QualityEvaluation, error)

		// List lists quality evaluations for a space.
		List(ctx context.Context, spaceID int64, filter *types.QualityEvaluationFilter) ([]*types.QualityEvaluation, error)

		// Count returns the count of quality evaluations for a space.
		Count(ctx context.Context, spaceID int64, filter *types.QualityEvaluationFilter) (int64, error)

		// Summary returns aggregate quality statistics for a space.
		Summary(ctx context.Context, spaceID int64) (*types.QualitySummary, error)
	}

	// ErrorTrackerStore defines the error tracker data storage.
	ErrorTrackerStore interface {
		// CreateOrUpdateErrorGroup creates a new or updates an existing error group.
		CreateOrUpdateErrorGroup(ctx context.Context, errorGroup *types.ErrorGroup) error

		// CreateErrorOccurrence creates a new error occurrence.
		CreateErrorOccurrence(ctx context.Context, occurrence *types.ErrorOccurrence) error

		// FindByIdentifier returns an error group by identifier.
		FindByIdentifier(ctx context.Context, spaceID int64, identifier string) (*types.ErrorGroup, error)

		// FindByFingerprint returns an error group by fingerprint.
		FindByFingerprint(ctx context.Context, spaceID int64, fingerprint string) (*types.ErrorGroup, error)

		// Count returns the count of error groups matching the options.
		Count(ctx context.Context, spaceID int64, opts types.ErrorTrackerListOptions) (int64, error)

		// List returns a paginated list of error groups.
		List(ctx context.Context, spaceID int64, opts types.ErrorTrackerListOptions) ([]types.ErrorGroup, error)

		// UpdateStatus updates the status of an error group.
		UpdateStatus(ctx context.Context, errorGroupID int64, status types.ErrorGroupStatus, updatedBy int64) error

		// UpdateAssignment updates the assigned user of an error group.
		UpdateAssignment(ctx context.Context, errorGroupID int64, assignedTo *int64) error

		// ListOccurrences returns a paginated list of error occurrences for an error group.
		ListOccurrences(ctx context.Context, errorGroupID int64, limit int, offset int) ([]types.ErrorOccurrence, error)

		// GetSummary returns summary statistics for error groups in a space.
		GetSummary(ctx context.Context, spaceID int64) (*types.ErrorTrackerSummary, error)
	}

	// HealthCheckStore defines the health check data storage.
	HealthCheckStore interface {
		// Find returns a health check given an ID
		Find(ctx context.Context, id int64) (*types.HealthCheck, error)

		// FindByIdentifier returns a health check given a space ID and identifier
		FindByIdentifier(ctx context.Context, spaceID int64, identifier string) (*types.HealthCheck, error)

		// Create creates a new health check
		Create(ctx context.Context, hc *types.HealthCheck) error

		// Update tries to update a health check
		Update(ctx context.Context, hc *types.HealthCheck) error

		// UpdateOptLock updates the health check using the optimistic locking mechanism.
		UpdateOptLock(
			ctx context.Context, hc *types.HealthCheck,
			mutateFn func(hc *types.HealthCheck) error,
		) (*types.HealthCheck, error)

		// List lists the health checks in a given space.
		List(ctx context.Context, spaceID int64, filter types.ListQueryFilter) ([]*types.HealthCheck, error)

		// ListAll lists all the health checks in a given space.
		ListAll(ctx context.Context, spaceID int64) ([]*types.HealthCheck, error)

		// Delete deletes a health check given an ID.
		Delete(ctx context.Context, id int64) error

		// DeleteByIdentifier deletes a health check given a space ID and identifier.
		DeleteByIdentifier(ctx context.Context, spaceID int64, identifier string) error
	...
	HealthCheckResultStore interface {
		// Find returns a health check result given an ID
		Find(ctx context.Context, id int64) (*types.HealthCheckResult, error)

		// Create creates a new health check result
		Create(ctx context.Context, result *types.HealthCheckResult) error

		// ListByHealthCheckID lists results for a specific health check
		ListByHealthCheckID(ctx context.Context, healthCheckID int64, limit int) ([]*types.HealthCheckResult, error)

		// CountByStatus counts results with a specific status for a health check
		CountByStatus(ctx context.Context, healthCheckID int64, status string) (int64, error)

		// CountTotal counts all results for a health check
		CountTotal(ctx context.Context, healthCheckID int64) (int64, error)

		// GetAverageResponseTime gets average response time in milliseconds for a health check
		GetAverageResponseTime(ctx context.Context, healthCheckID int64, hours int) (int64, error)
