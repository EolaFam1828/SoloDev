CREATE TABLE health_checks (
    ...
    hc_last_checked_at INTEGER NOT NULL DEFAULT 0,
    hc_last_response_time INTEGER NOT NULL DEFAULT 0,
    hc_consecutive_failures INTEGER NOT NULL DEFAULT 0,
    hc_created_by INTEGER NOT NULL,
    hc_created INTEGER NOT NULL,
    hc_updated INTEGER NOT NULL,
    hc_version INTEGER NOT NULL DEFAULT 0,
    UNIQUE (hc_space_id, hc_identifier),
    FOREIGN KEY (hc_space_id) REFERENCES spaces(space_id) ON DELETE CASCADE,
    FOREIGN KEY (hc_created_by) REFERENCES principals(principal_id) ON DELETE NO ACTION
);
