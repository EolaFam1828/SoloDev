...
// HandleRuleToggle is an HTTP handler for toggling a quality rule's enabled state.
func HandleRuleToggle(ctrl *qualitygate.Controller) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		in := new(qualitygate.ToggleRuleInput)
		...
		rule, err := ctrl.ToggleRule(ctx, session, spaceRef, ruleIdentifier, in)
		if err != nil {
			render.TranslatedUserError(ctx, w, err)
			return
		}
